"""잠실/송파 재건축 투자 다차원 심층분석 엑셀 생성기 v3
...
def sheet_dashboard(wb):
    ...
    # ROI(중립)
    rd = ["ROI 중립(%)"] + [calc_roi(ROI[cpx], ROI[cpx]["신축가_중"]) for cpx in COMPLEXES] + [""]
    r = write_row(ws, r, rd, fill=white_fill)
    ...
    ws.cell(row=r, column=2, value="ROI 중립(%)").font = sub_header_font
    r += 1
    for ci, cpx in enumerate(COMPLEXES):
        ws.cell(row=r, column=1, value=SHORT[ci])
        ws.cell(row=r, column=2, value=calc_roi(ROI[cpx], ROI[cpx]["신축가_중"]))
        r += 1

    chart = BarChart()
    chart.type = "col"
    chart.title = "단지별 ROI(중립) 비교 (%)"
...
def sheet_final(wb):
    ...
    conclusions = [
        ...
        "",
        "3. [밸런스] 장미1\xb72차: 한강조망+적정가+5165세대 대단지",
        "   \u2192 26.5억 매입, 42억+ 기대, 리스크 대비 수익 균형",
        "",
        "4. [확정시공사] 우성1\xb72\xb73차: GS건설 확정, 자이 브랜드",
        ...
        "",
        "8. [분산전략] 우성4차(22억)+장미1\xb72차(26.5억) 동시매입 고려",
...
def main():
    ...
    output = "C:/Devs/KRE/잠실_송파_재건축_투자분석_v3.xlsx"
    wb.save(output)
    print(f"\n완료! {output}")
