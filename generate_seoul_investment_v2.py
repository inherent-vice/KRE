#!/usr/bin/env python3
"""서울 25개 자치구 + 구내 상위단지 심층분석 생성기 (v2)
...
실행 예시
python generate_seoul_investment_v2.py --months 12 --all-profiles
"""
...
def run_complex_expert_scoring(metrics: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def value_expert(m: dict[str, Any]) -> float:
        ...
        s_pvs = percentile_rank(m.get("price_vs_seoul"), pvs_all, reverse=True)
        return 0.35 * s_jeonse + 0.25 * s_yield + 0.25 * s_pvd + 0.15 * s_pvs

    def liquidity_expert(m: dict[str, Any]) -> float:
        s_liq = percentile_rank(m.get("avg_trade_3m"), liq_all)
        s_latest = percentile_rank(float(m.get("latest_trade_count") or 0), liq_all)
        return 0.60 * s_liq + 0.40 * s_latest

    def stability_expert(m: dict[str, Any]) -> float:
        s_vol = percentile_rank(m.get("vol_ann"), vol_all, reverse=True)
        s_dd = percentile_rank(m.get("max_drawdown"), dd_all, reverse=True)
        dq = m.get("data_quality") or 0.0
        return 0.40 * s_vol + 0.30 * s_dd + 0.30 * dq
...
def add_sheet_scenario_method(wb, profile: str):
    ...
    steps = [
        ...
        "6) 단지 레벨은 구 점수(거시) 20% + 단지 점수(미시) 80%로 결합해 과적합 완화",
        "7) 전세환산 수익률(또는 실거주 효용)을 보유수익으로 반영해 과도한 비관 편향을 완화",
        "8) 프로필별 LTV/대출금리/취득세/보유비용/양도세를 적용해 레버리지 ROI 계산",
        "9) 데이터 품질점수(DataQ)로 표본이 낮은 구/단지의 해석신뢰도 경고",
    ]
...
def add_sheet_complex_rank(wb, complex_rows: list[dict[str, Any]], profile: str):
    ...
    r = write_headers(
        ws,
        r,
        ["전체순위", "구내순위", "구", "단지", "총점", "Core점수", "최근매매(억)", "12M성장", "전세가율", "변동성", "최근거래", "중립ROI", "비관ROI", "DataQ"],
    )
    ...
    set_widths(ws, [8, 8, 11, 26, 9, 9, 10, 10, 9, 9, 9, 10, 10, 8])
...
def main() -> int:
    ...
    cache_path = out_dir / f"seoul_market_cache_v2_{months[0]}_{months[-1]}_{int(args.area_min)}_{int(args.area_max)}.json"
    ...
    for profile in profiles:
        ...
        out_path = out_dir / f"서울_전체_아파트_심층분석_v2_{profile}.xlsx"
        wb.save(out_path)
    ...
    master_path = out_dir / "서울_전체_아파트_심층분석_v2_통합.xlsx"
