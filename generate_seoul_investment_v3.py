#!/usr/bin/env python3
"""서울 25개 자치구 + 구내 상위단지 심층분석 생성기 (v3)
...
실행 예시
python generate_seoul_investment_v3.py --months 12 --all-profiles
"""
...
def run_complex_expert_scoring(metrics: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def trend_expert(m: dict[str, Any]) -> float:
        ...
        return 0.55 * s12 + 0.30 * srel + 0.15 * sw
    ...
    def value_expert(m: dict[str, Any]) -> float:
        ...
        return 0.30 * s_jeonse + 0.25 * s_yield + 0.20 * s_pvd + 0.10 * s_pvs + 0.15 * conf

    def liquidity_expert(m: dict[str, Any]) -> float:
        s_liq = percentile_rank(m.get("avg_trade_3m"), liq_all)
        s_latest = percentile_rank(float(m.get("latest_trade_count") or 0), liq_all)
        conf = confidence_score(m)
        return 0.55 * s_liq + 0.30 * s_latest + 0.15 * conf
    ...
    def stability_expert(m: dict[str, Any]) -> float:
        ...
        dq = m.get("data_quality") or 0.0
        conf = confidence_score(m)
        return 0.32 * s_vol + 0.28 * s_dd + 0.22 * dq + 0.18 * conf
...
def scenario_for_district(m: dict[str, Any], profile: str) -> dict[str, Any]:
    ...
    base_annual -= unc_pen * 0.02

    vol_ann = m.get("vol_ann") or 0.08
    vol_ann = vol_ann * (1.0 + unc_pen * 1.4)
    ...
    stress_up_rate = loan_rate + DEFAULT_SCENARIO_RATE_SHOCK
    stress_dn_rate = max(0.0, loan_rate - DEFAULT_SCENARIO_RATE_SHOCK)
...
def add_sheet_scenario_method(wb, profile: str):
    ...
    steps = [
        ...
        "6) 단지 성장률은 구 성장률로 shrinkage(표본가중)해 노이즈를 축소",
        "7) 단지 레벨 총점은 구 점수(거시) 20% + 단지 점수(미시) 80%로 결합",
        "8) 전세환산 수익률(또는 실거주 효용)을 보유수익으로 반영해 과도한 비관 편향을 완화",
        "9) 프로필별 LTV/대출금리/취득세/보유비용/양도세를 적용해 레버리지 ROI 계산",
        "10) 데이터 품질점수(DataQ)+신뢰도(high/medium/low)로 불확실성 패널티를 반영",
        "11) 금리 ±1%p 민감도(중립ROI)로 대출환경 변화 충격을 별도 측정",
    ]
    ...
    rows = [
        ...
        ["Step5", "노이즈축소", "g_adj=w*g+(1-w)*g_district", "표본작은 단지 안정화", "과도한 평활", "w 상한/하한"],
        ["Step6", "재무반영", "ROI=(NetGain-이자-세금)/자기자본", "투자자별 현실성", "세법변경", "가정표 분리"],
        ["Step7", "민감도", "ROI(r+1%p), ROI(r-1%p)", "금리충격 대비", "단순 금리가정", "스트레스 폭 고정"],
    ]
    for x in rows:
...
def add_sheet_complex_rank(wb, complex_rows: list[dict[str, Any]], profile: str):
    ...
    r = write_headers(
        ws,
        r,
        ["전체순위", "구내순위", "구", "단지", "총점", "Core점수", "최근매매(억)", "12M성장", "전세가율", "변동성", "최근거래", "중립ROI", "비관ROI", "DataQ", "신뢰도", "매매표본합"],
    )
    ...
    set_widths(ws, [8, 8, 11, 26, 9, 9, 10, 10, 9, 9, 9, 10, 10, 8, 8, 9])
...
def add_sheet_complex_confidence(wb, complex_rows: list[dict[str, Any]]):
    ...
    r = write_headers(
        ws,
        r,
        ["순위", "구", "단지", "신뢰도", "DataQ", "매매표본합", "최근매매표본", "매매커버리지", "불확실성패널티", "샘플가중치", "중립ROI", "비고"],
    )
    for x in complex_rows:
        sc = x["scenario"]
    ...
    set_widths(ws, [7, 10, 26, 9, 8, 10, 10, 10, 12, 10, 10, 10])
...
def add_sheet_complex_sensitivity(wb, complex_rows: list[dict[str, Any]], profile: str):
    ...
    r = add_common_header(
        ws,
        f"단지 금리 민감도 ({profile})",
        f"중립 시나리오 기준 대출금리 ±{int(DEFAULT_SCENARIO_RATE_SHOCK*100)}%p 영향",
        11,
    )
    r = write_headers(
        ws,
        r,
        ["순위", "구", "단지", "중립ROI", "금리+1%p ROI", "금리-1%p ROI", "민감도폭", "LTV", "기준금리", "신뢰도", "비고"],
    )
    ...
    set_widths(ws, [7, 10, 26, 10, 11, 11, 10, 8, 9, 8, 10])
...
def main() -> int:
    ...
    cache_path = out_dir / f"seoul_market_cache_v3_{months[0]}_{months[-1]}_{int(args.area_min)}_{int(args.area_max)}.json"
    ...
    for profile in profiles:
        ...
        out_path = out_dir / f"서울_전체_아파트_심층분석_v3_{profile}.xlsx"
    ...
    master_path = out_dir / "서울_전체_아파트_심층분석_v3_통합.xlsx"
    ...
    if not args.no_universe:
        ...
        for profile in profiles:
            ...
            out_u = out_dir / f"서울_전체_아파트_실전유니버스_v3_{profile}.xlsx"
        ...
        if crows_uni_i:
            ...
            out_ui = out_dir / "서울_전체_아파트_실전유니버스_v3_통합.xlsx"
